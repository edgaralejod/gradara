...
Open **Examples**, choose **Use example** under **DC motor**, then press **Run**. This creates an independent saved document. The default plot compares the speed request and measured shaft speed over four seconds.
...
The executable template is [dc.json](examples/dc.json). Physical wrappers are in [server/modelica.py](../server/modelica.py), and the real-engine motor checks are in [tests/test_modelica.py](../tests/test_modelica.py). Use **Export → Modelica source** to inspect the code emitted from your model.
