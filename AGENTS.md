...
| --- | --- | --- |
| Workspace and panels | `app/page.tsx`, `app/engineering.css`, `components/gradara/` | UI audit; component agent guidance. |
| Block visuals / library | `block-face.tsx`, `block-symbol.tsx`, `lib/gradara/block-design.ts` | `BLOCK_DESIGN.md`, `docs/blocks/AGENT_BLOCK_GUIDE.md`. |
| Wiring / selection / routing | `lib/gradara/net-*.ts`, `routing.ts`, `selection.ts`, canvas/net components | `WIRING.md`, `WIRING_AUDIT.md`, model format. |
...
- Keep the canvas, library, and catalog on shared `BlockFace` rendering and shared port geometry. Refresh `BLOCK_AUDIT.md` with `npm run audit:blocks` when catalog or size rules change.
