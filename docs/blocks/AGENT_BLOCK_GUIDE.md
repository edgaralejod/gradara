...
Read [the block design contract](../../BLOCK_DESIGN.md) before adding or changing block presentation. This applies to built-in catalog entries and agent-generated equation blocks.
...
7. Run `npm run audit:blocks` to refresh the inventory. Run `npm run typecheck` and the existing frontend tests for any geometry/insertion changes. Use focused numerical validation if behavior changes; a visual-only edit should not require a new solver.
